...
export default function ConnexionPage() {
  ...
  const handleLoginSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!identifier || !password) {
      setIsError(true);
      setMessage("Merci de renseigner vos identifiants.");
      return;
    }
    ...
    if (result?.error) {
      setIsError(true);
      setMessage("Identifiants incorrects. Merci de réessayer.");
    } else {
  ...
  const handleRegisterSubmit = async (
    ...
    if (!body.password || !body.passwordConfirm) {
      setIsError(true);
      setMessage("Merci de saisir et confirmer votre mot de passe.");
      return;
    }
    ...
    // Succès : on repasse sur l’onglet login avec un message
    setActiveTab("login");
    setIsError(false);
          ...
          <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
            <div className="grid gap-10 lg:gap-16 md:grid-cols-[1.1fr,0.9fr] items-start">
              ...
              <div className="border border-zinc-200 rounded-2xl shadow-sm bg-white/95 backdrop-blur-sm">
                ...
                {message && (
                  <div
                    className={`px-5 sm:px-6 pt-3 text-xs sm:text-sm ${
