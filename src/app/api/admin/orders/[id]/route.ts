...
async function requireAdmin() {
  // ✅ Si tu es en database sessions, getToken() peut ne pas marcher.
  // On passe donc par getServerSession()
...
export async function PATCH(
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      // ✅ Si un admin passe la commande en CONFIRMED,
      // on considère que le paiement est validé (comme si PayDunya l’avait confirmé)
      const finalUpdates = { ...updates } as any;
      ...
      // Optionnel : cohérence si annulation
      if (
        body.status === "CANCELLED" &&
        previousStatus !== "CANCELLED" &&
        previousPaymentStatus !== "PAID"
